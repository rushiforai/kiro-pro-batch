"""
Builder ID Registration/Login - Automation using BitBrowser + DrissionPage
Direct translation of register.py Playwright flow
"""
import time
import random
import secrets
...
def _wait_for_code(code_link, timeout=120):
    """Poll code API to get verification code (bypass proxy, need to include cookie)"""
    import urllib3
    urllib3.disable_warnings()
    print(f"  [Code API] Polling verification code...")
    headers = {
    ...
    while time.time() - start < timeout:
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                # Format: {"code": 0, "message": "SUCCESS", "data": {"code": "871435"}}
                if data.get("code") == 0 and data.get("data"):
                    code = data["data"].get("code", "")
                    if code and str(code).isdigit():
                        print(f"  [Code API] Received verification code: {code}")
                        return str(code)
                # Compatible with: {"code": "324640"}
                raw_code = data.get("code", "")
                if isinstance(raw_code, str) and raw_code.isdigit() and len(raw_code) >= 4:
                    print(f"  [Code API] Received verification code: {raw_code}")
                    return raw_code
        except Exception:
            pass
        time.sleep(5)
    print("  [Code API] Timeout - no verification code received")
    return None


def login(email, code_link, proxies, window_index=0):
    """
    Builder ID registration/login flow (translated from register.py)
    Returns (token_info, page, browser_id)
    """
    _, session_id = get_proxies_with_check()
    browser_id = browser.create(session_id, f"bid_{email[:10]}")
    print(f"  [BuilderID] BitBrowser created: {browser_id[:16]}...")
    ...
    try:
        ...
        # 2. Click Builder ID button
        bid_btn = page.ele('text:Builder ID', timeout=15)
        if not bid_btn:
            bid_btn = page.ele('xpath://button[contains(., "Builder ID")]', timeout=5)
        if not bid_btn:
            raise RuntimeError("Builder ID button not found")
        bid_btn.click()
        print(f"  [BuilderID] Builder ID button clicked")
        time.sleep(5)

        # 3. Wait for redirect to signin.aws, enter email
        for _ in range(15):
        ...
        print(f"  [BuilderID] Email entered: {email}")
        time.sleep(1)
        ...
        page.run_js("""
            ...
            if (btn) btn.click();
        """)
        print(f"  [BuilderID] Continue clicked")
        time.sleep(6)

        # 5. Check if registering or logging in
        for _ in range(10):
            if "profile.aws" in page.url:
                break
            time.sleep(2)

        # Check if password field appears (account already exists)
        pwd_check = page.ele('xpath://input[@type="password"]', timeout=3)
        if pwd_check and "profile.aws" not in page.url:
            print(f"  [BuilderID] Account exists, logging in with password...")
        ...
        if name_input:
            ...
            print(f"  [BuilderID] Name entered: {name}")
        else:
            raise RuntimeError("Name input not found")
        time.sleep(1)
        ...
        page.run_js("""
            ...
            if (btn) btn.click();
        """)
        print(f"  [BuilderID] Next clicked")
        time.sleep(5)

        # 7. Wait for verification code input, get code
        ...
        if not code:
            raise RuntimeError(f"Failed to get verification code: {email}")
        code_input.input(code)
        print(f"  [BuilderID] Verification code entered: {code}")
        time.sleep(1)
        ...
        page.run_js("""
            ...
            if (btn) btn.click();
        """)
        print(f"  [BuilderID] Verify clicked")
        time.sleep(5)
        ...
        print(f"  [BuilderID] Password entered")
        time.sleep(1)

        # 10. Submit and wait for redirect
        page.run_js("""
            var btn = document.querySelector('[data-testid="test-primary-button"]');
            ...
            if (btn) btn.click();
        """)
        print(f"  [BuilderID] Waiting for redirect to app.kiro.dev...")
...
def _extract_token(page, browser_id):
    """Extract token info from logged-in page, proactively get profileArn and csrfToken"""
    time.sleep(3)
    ...
    # If profileArn or csrfToken not obtained, call API actively
    if not profile_arn or not csrf_token:
        ...
        try:
            ...
            print(f"  [BuilderID] Failed to get profileArn: {e}")

    # Builder ID user's profileArn is a fixed value
    if not profile_arn:
        profile_arn = "arn:aws:codewhisperer:us-east-1:000000000000:profile/PLACEHOLDER"

    print(f"  [BuilderID] Registration/Login successful! UserId: {token_data['user_id'][:20]}...")
    if profile_arn:
